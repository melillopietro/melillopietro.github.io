...
.NOTES
    ...
    Some packages, including Microsoft 365 Apps / Office, may require a valid license/account. v3.2 records WinGet stdout/stderr, command line, exit code, source, per-package logs and supports alternate package IDs for renamed WinGet manifests.
#>

#region === BASE CONFIGURATION ===
$ErrorActionPreference = 'Stop'
$script:AppName = 'SetupHub'
$script:AppVersion = '3.2'
...
$script:Text = @{
    it = @{
        ...
        Footer = 'Creato da Pietro Melillo | Powered by WinGet | SetupHub v3.2'
    }
    en = @{
        ...
        Footer = 'Created by Pietro Melillo | Powered by WinGet | SetupHub v3.2'
    }
}
...
$installPackages = @(
    ...
    # Virtualization / VPN
    @{ Name = 'VMware Workstation Pro'; Id = 'VMware.WorkstationPro'; Category = 'Virtualization'; Checked = $true; Profiles = @('Developer','Cybersecurity'); VerifiedUrl = 'https://winget.run/pkg/VMware/WorkstationPro'; Notes = 'May require Broadcom/VMware availability and installer compatibility.' }
    @{ Name = 'VMware Workstation Player'; Id = 'VMware.WorkstationPlayer'; Category = 'Virtualization'; Checked = $false; Profiles = @('Developer','Cybersecurity'); VerifiedUrl = 'https://winget.run/pkg/VMware/WorkstationPlayer'; Notes = 'Alternative to Workstation Pro when Pro installer fails.' }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid>
        ...
        <Border Grid.Row="4" Background="#181825" Padding="10,6">
            <TextBlock x:Name="lblFooter" Text="Creato da Pietro Melillo | Powered by WinGet | SetupHub v3.2" Foreground="#6c7086" FontSize="10" HorizontalAlignment="Center"/>
        </Border>
    </Grid>
</Window>
"@
...
$btnStart.Add_Click({
    ...
    $psCmd.AddScript({
        ...
        function Get-MeaningfulMessage {
            ...
            $combined = (($StdErr + "`n" + $StdOut) -split "`r?`n" | ForEach-Object { $_.Trim() } | Where-Object { $_.Length -gt 0 })
            $preferred = $combined | Where-Object { $_ -match '(?i)(error|errore|failed|failure|not found|no package|no applicable|hash|installer|license|agreement|cancel|denied|access|requires|reboot|already installed|non trovato|impossibile|annull)' }
            ...
            try {
                ...
                if ([string]::IsNullOrWhiteSpace($resolvedId)) {
                    ...
                    $results.Add([pscustomobject]@{
                        Timestamp=(Get-Date).ToString('s'); Action='Install'; Name=$item.Name; Id=$item.Id; ResolvedId=''; Category=$item.Category; Source=$source;
                ...
                if ($run.StdErr) { $run.StdErr -split "`r?`n" | Where-Object { $_.Trim().Length -gt 0 } | ForEach-Object { Write-LogLine "    STDERR: $_" } }
                ...
                $results.Add([pscustomobject]@{
                    Timestamp=(Get-Date).ToString('s'); Action='Install'; Name=$item.Name; Id=$item.Id; ResolvedId=$resolvedId; Category=$item.Category; Source=$source;
                ...
                $results.Add([pscustomobject]@{ Timestamp=(Get-Date).ToString('s'); Action='Install'; Name=$item.Name; Id=$item.Id; ResolvedId=''; Category=$item.Category; Source=$errSource; Status='Exception'; ExitCode=''; DurationSeconds=[math]::Round(((Get-Date)-$start).TotalSeconds,2); Message=$_.Exception.Message; Command=''; WinGetLog=''; StdOut=''; StdErr='' }) | Out-Null
            }
            Write-LogLine ''
        }
        ...
        foreach ($item in $selectedBloat) {
            ...
            $start = Get-Date
            try {
                $run = Invoke-WinGetProcess -Action 'uninstall' -Id $item.Id -SkipSilent $false -Source 'winget' -PackageName $item.Name
                if ($run.StdOut) { $run.StdOut -split "`r?`n" | Where-Object { $_.Trim().Length -gt 0 } | ForEach-Object { Write-LogLine "    $_" } }
                if ($run.StdErr) { $run.StdErr -split "`r?`n" | Where-Object { $_.Trim().Length -gt 0 } | ForEach-Object { Write-LogLine "    STDERR: $_" } }
                if ($run.ExitCode -eq 0) {
                ...
                $results.Add([pscustomobject]@{
                    Timestamp=(Get-Date).ToString('s'); Action='Uninstall'; Name=$item.Name; Id=$item.Id; Category='Windows App'; Source='local';
                    Status=$status; ExitCode=$run.ExitCode; DurationSeconds=[math]::Round(((Get-Date)-$start).TotalSeconds,2);
                    Message=$message; Command=$run.Args; WinGetLog=$run.LogPath; StdOut=$run.StdOut; StdErr=$run.StdErr
                }) | Out-Null
            } catch {
                ...
                $results.Add([pscustomobject]@{ Timestamp=(Get-Date).ToString('s'); Action='Uninstall'; Name=$item.Name; Id=$item.Id; Category='Windows App'; Source='local'; Status='Exception'; ExitCode=''; DurationSeconds=[math]::Round(((Get-Date)-$start).TotalSeconds,2); Message=$_.Exception.Message; Command=''; WinGetLog=''; StdOut=''; StdErr='' }) | Out-Null
            }
            Write-LogLine ''
        }
        ...
        if ($createReport) {
            ...
            $html += ($results | Select-Object Timestamp,Action,Name,Id,Category,Source,Status,ExitCode,DurationSeconds,Message,Command,WinGetLog | ConvertTo-Html -Fragment)
            $html += '</div></body></html>'
        ...
        if ($createReport) { Write-LogLine "  $(TT 'ReportSaved'): $htmlPath"; Write-LogLine "  CSV: $csvPath"; Write-LogLine "  JSON: $jsonPath"; Write-LogLine "  TXT Log: $logPath"; Write-LogLine "  WinGet logs: $wingetLogFolder" }
        Write-LogLine '========================================='
