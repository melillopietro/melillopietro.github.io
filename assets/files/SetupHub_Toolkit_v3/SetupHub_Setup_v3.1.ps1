...
.NOTES
    ...
    Some packages, including Microsoft 365 Apps / Office, may require a valid license/account. v3.1 records WinGet stdout/stderr, command line, exit code, source and per-package logs.
#>
    ...
    $psCmd.AddScript({
        ...
        foreach ($item in $selectedInstalls) {
            ...
            try {
                ...
                Write-LogLine "    $(TT 'PackageCheck'): source=$source"
                $show = Invoke-WinGetProcess -Action 'show' -Id $item.Id -SkipSilent $true -Source $source -PackageName $item.Name
                if ($show.ExitCode -ne 0) {
                    ...
                    $results.Add([pscustomobject]@{
                        Timestamp=(Get-Date).ToString('s'); Action='Install'; Name=$item.Name; Id=$item.Id; Category=$item.Category; Source=$source;
                ...
                $run = Invoke-WinGetProcess -Action 'install' -Id $item.Id -SkipSilent ([bool]$item.SkipSilent) -Source $source -PackageName $item.Name
                ...
                $results.Add([pscustomobject]@{ Timestamp=(Get-Date).ToString('s'); Action='Install'; Name=$item.Name; Id=$item.Id; Category=$item.Category; Source=$errSource; Status='Exception'; ExitCode=''; DurationSeconds=[math]::Round(((Get-Date)-$start).TotalSeconds,2); Message=$_.Exception.Message; Command=''; WinGetLog=''; StdOut=''; StdErr='' }) | Out-Null
            }
            Write-LogLine ''
        }
