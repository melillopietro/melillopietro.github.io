---
title: Certifications
icon: fas fa-certificate
order: 2
---

This page provides an overview of my professional certifications in cybersecurity, information security management, and cyber threat intelligence.  
These credentials reflect both executive-level governance competencies and hands-on technical expertise, aligned with my roles in industry, research, and academia.

## Executive & Security Governance

- **Certified Chief Information Security Officer (CCISO)**  
  Focused on security leadership, risk management, governance frameworks, and strategic alignment between cybersecurity and business objectives.

## Cyber Threat Intelligence & Incident Response

- **Certified Threat Intelligence Analyst (CTIA)** – EC-Council  
  Structured threat intelligence lifecycle, adversary analysis, and intelligence-driven security operations.

- **Certified Incident Handler (ECIH)** – EC-Council  
  Incident detection, response coordination, containment strategies, and post-incident analysis.

## Offensive Security & Technical Foundations

- **Certified Ethical Hacker (CEH)** – EC-Council  
  Foundations of offensive security, adversarial techniques, and defensive countermeasures.

- **Red Teaming Active Directory for Beginners** 
  Practical introduction to Active Directory attack paths, privilege escalation, and lateral movement techniques.

## SIEM & Security Monitoring Platforms

- **IBM QRadar SIEM Foundation Certification** – 
- **IBM QRadar SIEM Advanced Certification** – IBM  

  Design, deployment, and advanced use of SIEM architectures for log correlation, detection engineering, and enterprise security monitoring.

---

> Certifications are continuously maintained and updated through professional practice, ongoing training, and alignment with evolving regulatory and threat landscapes.
